...
const ValueTriad: React.FC = () => {
  return (
    <section id="philosophy" className="py-24 relative overflow-hidden border-t border-white/5">
        {/* Radial sheen */}
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] bg-electric/5 blur-[120px] rounded-full pointer-events-none"></div>
        
        {/* Watermark Logo - Isotipo del triángulo como elemento decorativo de fondo */}
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        <div className="text-center mb-16">
          <ScrollReveal>
             ...
             <h3 className="text-3xl md:text-5xl font-bold text-white">Ingeniería aplicada al negocio</h3>
          </ScrollReveal>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {pillars.map((pillar, index) => (
            <ScrollReveal key={index} delay={index * 150}>
              <div className="h-full flex flex-col items-center text-center p-8 rounded-2xl hover:bg-white/[0.02] transition-colors border border-transparent hover:border-white/5 backdrop-blur-sm">
                <div className={`w-16 h-16 rounded-2xl ${pillar.bg} ${pillar.color} flex items-center justify-center mb-6 shadow-lg shadow-black/50`}>
                  {pillar.icon}
                </div>
